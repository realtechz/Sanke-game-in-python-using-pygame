...
class Snake:
    ...
    def move(self):
        ...
        if self.change_to == 'RIGHT' and self.direction == 'E':
            self.direction = 'S'
            self.change_to = ''
        if self.change_to == 'LEFT' and self.direction == 'E':
            self.direction = 'N'
            self.change_to = ''
        if self.change_to == 'RIGHT' and self.direction == 'S':
            self.direction = 'W'
            self.change_to = ''
        if self.change_to == 'LEFT' and self.direction == 'S':
            self.direction = 'E'
            self.change_to = ''
        if self.change_to == 'RIGHT' and self.direction == 'N':
            self.direction = 'E'
            self.change_to = ''
        if self.change_to == 'LEFT' and self.direction == 'N':
            self.direction = 'W'
            self.change_to = ''
        if self.change_to == 'RIGHT' and self.direction == 'W':
            self.direction = 'N'
            self.change_to = ''
        if self.change_to == 'LEFT' and self.direction == 'W':
            self.direction = 'S'
            self.change_to = ''
...
def draw_window(win, snake, score, deaths):
    ...
    win.blit(text, (WIN_WIDTH - 50 - text.get_width(), WIN_HEIGHT + 50))
    # generations
    score_label = STAT_FONT.render("Deaths: " + str(deaths - 1), True, black)
    win.blit(score_label, (50, WIN_HEIGHT + 50))
